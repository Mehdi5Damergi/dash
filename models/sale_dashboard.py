...
class SaleOrder(models.Model):
    ...
    def get_department(self, option):
        ...
        if option == 'sales_hourly':
            ...
            query = '''select  EXTRACT(hour FROM date_order at time zone 'utc' at time zone '{}') 
                       ...
                       AND sale_order.company_id = ''' + str(
                company_id) + ''' group by date_month '''
            query = query.format(user_tz)
            label = 'HOURS'
        elif option == 'sales_monthly':
            query = '''select  date_order::date as date_month,sum(amount_total) from sale_order where 
             EXTRACT(month FROM date_order::date) = EXTRACT(month FROM CURRENT_DATE) AND sale_order.company_id = ''' + str(
                company_id) + '''  group by date_month '''
            label = 'DAYS'
        else:
            query = '''select TO_CHAR(date_order,'MON')date_month,sum(amount_total) from sale_order where
             EXTRACT(year FROM date_order::date) = EXTRACT(year FROM CURRENT_DATE) AND sale_order.company_id = ''' + str(
                company_id) + ''' group by date_month'''
            label = 'MONTHS'
    ...
    def get_details(self):
        ...
        cr = self._cr
        cr.execute(
            """select payment_term.name ->>'en_US',sum(amount) from pos_payment inner join payment_term on 
            payment_term.id=pos_payment.payment_method_id group by payment_term.name ORDER 
            BY sum(amount) DESC; """)
        payment_details = cr.fetchall()
        cr.execute(
            '''select hr_employee.name,sum(sale_order.amount_paid) as total,count(sale_order.amount_paid) as orders 
            from sale_order inner join hr_employee on sale_order.user_id = hr_employee.user_id 
            where sale_order.company_id =''' + str(
    ...
    def get_the_top_customer(self, ):
        ...
        company_id = self.env.company.id
        query = '''select res_partner.name as customer,sale_order.partner_id,sum(sale_order.amount_paid) as amount_total from sale_order 
        inner join res_partner on res_partner.id = sale_order.partner_id where sale_order.company_id = ''' + str(
            company_id) + ''' GROUP BY sale_order.partner_id,
    ...
    def get_the_top_categories(self):
        ...
        from sale_order_line inner join product_product on product_product.id=sale_order_line.product_id  inner join 
        product_template on product_product.product_tmpl_id = product_template.id inner join product_category on 
        product_category.id =product_template.categ_id where sale_order_line.company_id = ''' + str(
